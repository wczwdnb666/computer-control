...
**默认应该走元素定位。** 靠截图量坐标再点像素，是最后手段，不是首选。

坐标是脆的：窗口一挪就失效、页面一滚就失效、分辨率一变就失效、界面语言一切换也可能失效。我实测时用坐标点击连栽三次——点到别的元素上、滚页之后坐标全废、误触发搜索浮层。

**元素定位不靠坐标，靠"这个控件叫什么"。** Windows UI Automation 能看到界面上每个控件的名称、类型、位置、是否可用：

```bash
python cc.py tree --process notepad --interactive     # 看这个窗口有什么
python cc.py find --win 记事本 --type EditControl     # 定位编辑区
python cc.py set-el "你好" --win 记事本 --type EditControl --paste
```

`click-el` 和 `set-el` **定位不到元素就直接报错退出，绝不盲点**。没有"差不多就点那儿"的兜底逻辑——这个拒绝本身就是它的价值。

但坐标不能丢，因为 UIA 看不到所有东西：

| 场景 | 原因 | 用什么 |
|---|---|---|
| 游戏、Canvas 界面 | 整个画面就是一个控件，内部没有元素 | 坐标 |
| Electron 应用（B站、Discord、VS Code） | Chromium 默认不向 UIA 暴露内部元素 | 坐标，或先开应用的无障碍支持 |
| 自绘 UI 的老软件 | 作者没实现 UIA 提供程序 | 坐标 |

**判断方法**：`tree --process <应用>` 如果只列出一两个元素，说明它不暴露内部结构，直接改用坐标。

元素定位需要可选依赖 `pip install uiautomation`。**不装也能用**，核心功能照旧零依赖，只有那四个元素命令会提示缺依赖。
